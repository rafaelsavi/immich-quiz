...
- [ ] **1. Client-Side Routing & Match URLs**
  - [ ] Implement History API router (`pushState`, `popstate`) supporting deep links:
    - `/` (Lobby & match setup)
    ...
    - `/play/{token}` (Challenge entry & player lobby)
    - `/stats` (Leaderboard & player statistics)
  - [ ] Add FastAPI backend catch-all route (`/{path:path}` -> `index.html`) so direct URL navigation and page refreshes never return 404.
  - [ ] Update PWA Service Worker (`sw.js`) navigation fallback to cache the app shell and handle offline/cached route transitions.
  - [ ] Refine in-game navigation guards (intercepting browser back/forward and accidental tab closes during active rounds).
...
- [ ] **3. Report Map or Date Inconsistencies**
...
- [ ] **4. Safety Against Attacks & Media Anti-Cheat**
...
- [ ] **5. Multiplayer Challenges (Async & Hybrid)**
...
- [ ] **6. Player Statistics & Match Replays**
...
- [ ] **8. Multiplayer Live Lounge (Real-Time Synchronous)**
...
- [ ] **9. Improve Audio Effects & Soundtrack**
