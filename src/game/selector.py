...
def is_asset_valid_for_batch(
    ...
    """Determine if a candidate photo satisfies distance and capture time separation constraints."""
    if location_mode:
...
async def _verify_and_get_asset(
    aid: str,
    state: MatchState,
    immich: ImmichClient,
) -> bool:
    ...
    """
    try:
        await immich.get_asset_bytes(state.setup.library_name, aid)
        return True
    except Exception:
        state.asset_pool.pop(aid, None)
        return False
...
async def select_round_asset(
    ...
    """Draw an unplayed asset for a single-photo round (Pinpoint mode)."""
    ...
    # Fallback if pool is constrained: pick any unplayed candidate that is readable
    for aid in candidates:
        if aid in state.asset_pool and await _verify_and_get_asset(aid, state, immich):
            return RoundAsset(asset_id=aid, answer=state.asset_pool[aid])
...
async def select_batch_round_assets(
    ...
    """Draw a batch of separated photos and generate map pins for Album Shuffle mode."""
    excluded = state.played_asset_ids | client_excluded
    ...
    if len(selected_assets) < count:
        for aid in shuffled_candidates:
            if aid not in selected_ids and aid in state.asset_pool:
                ans = state.asset_pool[aid]
    ...
    if state.setup.location_mode:
        ...
        for ra in assets:
            ...
            if abs(ra.answer.latitude) < 1e-6 and abs(ra.answer.longitude) < 1e-6:
                continue
            raw_pins.append(
                {'true_asset_id': ra.asset_id, 'latitude': ra.answer.latitude, 'longitude': ra.answer.longitude}
            )
